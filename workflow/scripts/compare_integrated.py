...
def get_bus_demand(n, busname, carrier_limit=False, carrier_limit_integration=False):
    ...
    if carrier_limit == False:
        ...
        if carrier_limit_integration == "inclusive":
            ...
            logging.error(
                "carrier_limit_integration must be 'inclusive' or 'exclusive'"
            )
    ...
    demand = demand[
        (demand.index.get_level_values(0) != "Store")
        & (demand.index.get_level_values(0) != "StorageUnit")
    ]
    ...
    demand = demand[
        (demand.index.get_level_values(1) != "H2")
        | (demand.index.get_level_values(3) == "H2 export bus")
    ]
...
def get_mean_prices(n):
    return n.buses_t.marginal_price.mean().groupby([n.buses.carrier]).first()
    ...
    for i in snakemake.input.networks:  # [0:limit]:
        ...
        for carrier in carriers:
            for w_opts in weighting_options[carrier]:
                ...
                marginals_nodal = (
                    n.buses_t.marginal_price[buses_sel] * demand
                ).sum() / demand.sum()

                # Step 4.1: Nodal weighted mean of marginal price
                marginals = (
                    (marginals_nodal * demand.sum() / demand.sum().sum()).sum().round(2)
                )

                # Step 4.2: Expense on carrier by consumer
                expense = (
                    (n.buses_t.marginal_price[buses_sel] * demand).sum().sum()
                    * n.snapshot_weightings.generators[0]
                    * (-1)
                    / 1e6
                )  # in Mio. €
        ...
        # Get the sum of expenses on local electricity and h2 export
        expense_df["exp_local_export"] = (
            expense_df["exp_AC_exclu_H2 El_all"]
            + expense_df["exp_H2_False_False_exportonly"]
        )
        ...
        Battery_GWh = (
            n.stores[(n.stores.carrier == "battery")].e_nom_opt.sum() / 1e3
        )  # in GWh
        EV_Battery_GWh = (
            n.stores[(n.stores.carrier == "Li ion")].e_nom_opt.sum() / 1e3
        )  # in GWh
        H2export_GWh = (
            n.stores[
                (n.stores.carrier == "H2") & (n.stores.bus == "H2 export bus")
        ...
        curtailmentrate_solar = (
            statistics.loc["Generator", "Solar"].Curtailment
            / statistics.loc["Generator", "Solar"].Dispatch
            * 100
        )
        curtailmentrate_wind = (
            statistics.loc["Generator", "Onshore Wind"].Curtailment
            / statistics.loc["Generator", "Onshore Wind"].Dispatch
            * 100
        )
        ...
        el_base_demand = (
            (loads.loc[:, ac_loads].sum() * n.snapshot_weightings.generators[0] / 1e6)
            .sum()
            .round(2)
        )  # TWh
        ...
        supply = (
            statistics.loc["Link", "H2 Electrolysis"].loc["Supply"].sum() / 1e6
        )  # in TWh
        ...
        mg_co2 = (
            n.buses_t.marginal_price.loc[:, n.buses[n.buses.carrier == "co2"].index]
            .mean()[0]
            .round(2)
        )
        ...
        pv_capex = (
            statistics.loc["Generator", "Solar"].loc["Capital Expenditure"].round(2)
            / 1e6
        )  # in Mio. €
        pv_p_nom_opt = (
            statistics.loc["Generator", "Solar"].loc["Optimal Capacity"].round(2) / 1e3
        )  # in GW
        pv_supply = (
            statistics.loc["Generator", "Solar"].loc["Supply"].sum() / 1e6
        )  # in TWh
        pv_cf = statistics.loc["Generator", "Solar"].loc["Capacity Factor"].round(2)

        onshore_capex = (
            statistics.loc["Generator", "Onshore Wind"]
            .loc["Capital Expenditure"]
            .round(2)
            / 1e6
        )  # in Mio. €
        onshore_p_nom_opt = (
            statistics.loc["Generator", "Onshore Wind"].loc["Optimal Capacity"].round(2)
            / 1e3
        )  # in GW
        onshore_supply = (
            statistics.loc["Generator", "Onshore Wind"].loc["Supply"].sum() / 1e6
        )  # in TWh
        onshore_cf = (
            statistics.loc["Generator", "Onshore Wind"].loc["Capacity Factor"].round(2)
        )

        coal_capex = (
            statistics.loc["Generator", "Coal"].loc["Capital Expenditure"].round(2)
            / 1e6
        )  # in Mio. €
        coal_p_nom_opt = (
            statistics.loc["Generator", "Coal"].loc["Optimal Capacity"].round(2) / 1e3
        )  # in GW
        coal_supply = (
            statistics.loc["Generator", "Coal"].loc["Supply"].sum() / 1e6
        )  # in TWh
        coal_cf = statistics.loc["Generator", "Coal"].loc["Capacity Factor"].round(2)

        ccgt_capex = (
            statistics.loc["Generator", "Combined-Cycle Gas"]
            .loc["Capital Expenditure"]
            .round(2)
            / 1e6
        )  # in Mio. €
        ccgt_p_nom_opt = (
            statistics.loc["Generator", "Combined-Cycle Gas"]
            .loc["Optimal Capacity"]
            .round(2)
            / 1e3
        )  # in GW
        ccgt_supply = (
            statistics.loc["Generator", "Combined-Cycle Gas"].loc["Supply"].sum() / 1e6
        )  # in TWh
        ccgt_cf = (
            statistics.loc["Generator", "Combined-Cycle Gas"]
            .loc["Capacity Factor"]
        ...
        ror_capex = (
            statistics.loc["Generator", "Run of River"]
            .loc["Capital Expenditure"]
            .round(2)
            / 1e6
        )  # in Mio. €
        ror_p_nom_opt = (
            statistics.loc["Generator", "Run of River"].loc["Optimal Capacity"].round(2)
            / 1e3
        )  # in GW
        ror_supply = (
            statistics.loc["Generator", "Run of River"].loc["Supply"].sum() / 1e6
        )  # in TWh
        ror_cf = (
            statistics.loc["Generator", "Run of River"].loc["Capacity Factor"].round(2)
        )

        oil_capex = (
            statistics.loc["Generator", "Oil"].loc["Capital Expenditure"].round(2) / 1e6
        )  # in Mio. €
        oil_p_nom_opt = (
            statistics.loc["Generator", "Oil"].loc["Optimal Capacity"].round(2) / 1e3
        )  # in GW
        oil_supply = (
            statistics.loc["Generator", "Oil"].loc["Supply"].sum() / 1e6
        )  # in TWh
        oil_cf = statistics.loc["Generator", "Oil"].loc["Capacity Factor"].round(2)

        ocgt_capex = (
            statistics.loc["Link", "Open-Cycle Gas"].loc["Capital Expenditure"].round(2)
            / 1e6
        )  # in Mio. €
        ocgt_p_nom_opt = (
            statistics.loc["Link", "Open-Cycle Gas"].loc["Optimal Capacity"].round(2)
            / 1e3
        )  # in GW
        ocgt_supply = (
            statistics.loc["Link", "Open-Cycle Gas"].loc["Supply"].sum() / 1e6
        )  # in TWh
        ocgt_cf = (
            statistics.loc["Link", "Open-Cycle Gas"].loc["Capacity Factor"].round(2)
        )

        ft_capex = (
            statistics.loc["Link", "Fischer-Tropsch"]
            .loc["Capital Expenditure"]
            .round(2)
            / 1e6
        )  # in Mio. €
        ft_p_nom_opt = (
            statistics.loc["Link", "Fischer-Tropsch"].loc["Optimal Capacity"].round(2)
            / 1e3
        )  # in GW
        ft_supply = (
            statistics.loc["Link", "Fischer-Tropsch"].loc["Supply"].sum() / 1e6
        )  # in TWh
        ft_cf = (
            statistics.loc["Link", "Fischer-Tropsch"].loc["Capacity Factor"].round(2)
        )

        # Capacity factor electrolysis
        cf_electrolysis = (
            statistics.loc["Link", "H2 Electrolysis"].loc["Capacity Factor"].round(2)
        )
        electrolysis_p_nom_opt = (
            statistics.loc["Link", "H2 Electrolysis"].loc["Optimal Capacity"].round(2)
            / 1e3
        )  # in GW
        electrolysis_supply = (
            statistics.loc["Link", "H2 Electrolysis"].loc["Supply"].sum() / 1e6
