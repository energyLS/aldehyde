...
def plot_costs():
    ...
    to_drop = df.index[df.max(axis=1) < snakemake.config["plot"]["costs_threshold"]]
    ...
    df.loc[new_index, new_columns].T.plot(
        ...
        ax=ax,
        stacked=True,
        color=[snakemake.config["plot"]["tech_colors"][i] for i in new_index],
    )
...
def plot_energy():
    ...
    to_drop = df.index[
        df.abs().max(axis=1) < snakemake.config["plot"]["energy_threshold"]
    ]
...
def plot_balances():
    ...
    for k, v in balances.items():
        ...
        to_drop = df.index[
            df.abs().max(axis=1) < snakemake.config["plot"]["energy_threshold"] / 10
        ]
