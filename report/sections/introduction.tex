...
\begin{enumerate}
    \item \textbf{Mitigation}: The (energy and non-energy) emission reduction varies between 0~--~100\% based on Morocco's current emissions of 72 Mt\coe (s. Sec. \ref{subsec:climate_targets}),
    \item \textbf{Export}: The hydrogen export volume varies from 0~--~120 TWh, in accordance with Moroccos hydrogen export ambitions of 114.7~TWh (s. Sec. \ref{subsec:strategy}) and
...
Chapter \nameref{sec:policyandtargets} presents Morocco's hydrogen strategy and climate targets and outlines potential conflicts and synergies between these two goals. Introducing the results along these three dimensions step-by-step, Chapter \nameref{sec:results} evaluates synergies and conflicts based on prices and total cost for domestic electricity consumers and hydrogen exporters. These results are contextualized along policy recommendations in Chapter \nameref{sec:discussion} and concluded in Chapter \nameref{sec:conclusion}. Chapter \nameref{sec:methods} presents the applied sector-coupled energy model and scenario dimensions required to expose the export-mitigation-regulation nexus.
